    ...
    public class AuthController {
        ...
        public ResponseEntity<?> login(@RequestBody LoginRequest loginRequest) {
            ...
            List<NguoiDung> nguoiDungList = userService.findByUserNameAndAuthProvider(username, AuthProvider.EMAIL);
            ...
            if (nguoiDung.getAuthProvider() != AuthProvider.EMAIL) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                        .body("Hãy đăng nhập bằng " + nguoiDung.getAuthProvider());
            }
            ...
            try {
                ...
                response.put("user", Map.of(
                        ...
                        "firstName", nguoiDung.getFirstName(),
                        "lastName", nguoiDung.getLastName(),
                        "role", nguoiDung.getRole().name(),
                        "displayName", nguoiDung.getFirstName() + " " + nguoiDung.getLastName(),
                        "avatarUrl", nguoiDung.getAvatarUrl(), // Thêm avatarUrl
                        "photoURL", nguoiDung.getAvatarUrl() != null && !nguoiDung.getAvatarUrl().isBlank()
                            ? nguoiDung.getAvatarUrl()
                            : "https://ui-avatars.com/api/?name=" + nguoiDung.getFirstName() + "+" + nguoiDung.getLastName()
                ));
        ...
        public ResponseEntity<?> updateProfile(@RequestBody UserDTOs userDTOs, Authentication authentication) {
            ...
            try {
                NguoiDung updatedNguoiDung = userService.updateMyOwnProfile(username,AuthProvider.EMAIL,userDTOs);
        ...
        public ResponseEntity<?> uploadAvatar(@RequestParam("file") MultipartFile file,
            ...
            try {
                NguoiDung nguoiDung = userService.uploadAvatarForUser(username, AuthProvider.EMAIL, file);
                return ResponseEntity.ok(Map.of("avatarUrl", nguoiDung.getAvatarUrl()));
            } catch (Exception e) {
        ...
        public ResponseEntity<?> loginWithFirebase(@RequestBody FirebaseLoginRequest request) {
            ...
            try {
                ...
                UserDetails userDetails = userService.loadUserByUsernameAndProvider(
                    nguoiDung.getUserName(), nguoiDung.getAuthProvider());
                String jwt = jwtUtil.generateToken(userDetails, nguoiDung.getAuthProvider().name());
                // Prepare response
                Map<String, Object> response = new HashMap<>();
