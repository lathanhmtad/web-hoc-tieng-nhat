...
public interface BlogRepository extends JpaRepository<Blog, UUID> {
    ...
    @Query("SELECT b FROM Blog b WHERE b.nguoiDung.user_id = :userId")
    ...
    @Query("SELECT COUNT(b) FROM Blog b WHERE b.nguoiDung.user_id = :userId")
    long countBlogsByUserId(@Param("userId") UUID userId);
    @Modifying
    @Transactional

    @Query("DELETE FROM Blog b WHERE b.nguoiDung.user_id = :userId")
