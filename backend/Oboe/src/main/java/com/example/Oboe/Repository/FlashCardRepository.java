...
public interface FlashCardRepository extends JpaRepository<BoThe, UUID> {
    ...
    @Query("SELECT f FROM BoThe f WHERE f.nguoiDung.user_id = :userId AND LOWER(f.term) LIKE LOWER(CONCAT('%', :term, '%'))")
    ...
    @Query("SELECT f FROM BoThe f WHERE f.nguoiDung.user_id = :userId ORDER BY f.created DESC")
    ...
    @Query("SELECT f FROM BoThe f WHERE f.set_id = :setId")
    Optional<BoThe> findById(@Param("setId") UUID setId);

    @Query(
            value = "SELECT new com.example.Oboe.DTOs.FlashcardSearchResultDTO(" +
                    "f.set_id, f.term, f.nguoiDung.userName, COUNT(c), f.nguoiDung.avatarUrl) " +
                    "FROM BoThe f LEFT JOIN f.mucThes c " +
                    "WHERE LOWER(f.term) LIKE LOWER(CONCAT('%', :keyword, '%')) " +
                    "GROUP BY f.set_id, f.description, f.nguoiDung.userName, f.nguoiDung.avatarUrl",
