  ...
  <div class="dictionary-management">
    ...
    <div class="search-filters">
      <div class="search-group">
        ...
        <input v-model="searchQuery" type="text" :placeholder="getSearchPlaceholder()" class="search-input">
      </div>
    ...
    <div class="content-area">
      ...
      <div v-if="activeTab === 'vocabulary'" class="vocabulary-section">
        ...
        <div v-if="searchQuery && searchQuery.trim()" class="search-results-info">
          <p>Tìm thấy <strong>{{ filteredVocabularyTotal }}</strong> từ vựng cho từ khóa "<strong>{{ searchQuery
          }}</strong>"</p>
          <p v-if="getSearchTotalPages() > 1" class="pagination-info">
            Hiển thị {{ searchCurrentPage * searchPageSize + 1 }} - {{ Math.min((searchCurrentPage + 1) *
              searchPageSize, filteredVocabularyTotal) }} trong tổng số {{ filteredVocabularyTotal }} kết quả
          </p>
        </div>

        <div class="items-grid">
          <div v-for="item in filteredVocabulary" :key="item.id" class="item-card vocabulary-card">
            ...
            <div class="card-content">
              ...
              <div class="meta-info">
                <span class="type">{{ getWordTypeLabel(item.type) }}</span>
          ...
          <div v-if="searchQuery && searchQuery.trim() && getSearchTotalPages() > 1" class="pagination">
            <button @click="prevSearchPage()" :disabled="searchCurrentPage === 0" class="pagination-btn">
            ...
            <button @click="nextSearchPage()" :disabled="searchCurrentPage >= getSearchTotalPages() - 1"
          ...
          <div v-if="totalPages > 1 && (!searchQuery || !searchQuery.trim())" class="pagination">
            <button @click="loadVocabulary(currentPage - 1)" :disabled="currentPage === 0" class="pagination-btn">
              <i class="fas fa-chevron-left"></i>
            </button>
            ...
            <button @click="loadVocabulary(currentPage + 1)" :disabled="isLastPage" class="pagination-btn">
              <i class="fas fa-chevron-right"></i>
            </button>
          </div>
        </div>
      ...
      <div v-if="activeTab === 'grammar'" class="grammar-section">
        ...
        <div v-else>
          <!-- Search Results Info -->
          <div v-if="searchQuery && searchQuery.trim()" class="search-results-info">
            <p>Tìm thấy <strong>{{ filteredGrammarTotal }}</strong> ngữ pháp cho từ khóa "<strong>{{ searchQuery
            }}</strong>"</p>
            <p v-if="getSearchTotalPages() > 1" class="pagination-info">
              Hiển thị {{ searchCurrentPage * searchPageSize + 1 }} - {{ Math.min((searchCurrentPage + 1) *
                searchPageSize, filteredGrammarTotal) }} trong tổng số {{ filteredGrammarTotal }} kết quả
            </p>
          </div>

          <div class="items-grid">
            <div v-for="item in filteredGrammar" :key="item.id" class="item-card grammar-card">
              <div class="card-header">
                <div class="grammar-info">
                  <h3 class="structure">{{ item.structure }}</h3>
                  <span style="color: #6c757d">{{ item.vietnamesePronunciation }}</span>
                </div>
                <div class="actions">
                  <button @click="editItem(item)" class="btn-edit" title="Chỉnh sửa">
                    <i class="fas fa-edit"></i>
              ...
              <div style="margin-top: 4px;">
                <div :class="['level-badge', item.jlptLevel.toLowerCase()]" style="margin-right: 4px;">{{
                  item.jlptLevel }}</div>
                <span class="grammar-type-badge" :class="item.grammarType">{{ getGrammarTypeName(item.grammarType)
                }}</span>
              </div>
          ...
          <!-- Regular Pagination -->
          <div v-if="totalPages > 1 && (!searchQuery || !searchQuery.trim())" class="pagination">
            <button @click="loadGrammar(currentPage - 1)" :disabled="currentPage === 0" class="pagination-btn">
              <i class="fas fa-chevron-left"></i>
            </button>
            ...
            <button @click="loadGrammar(currentPage + 1)" :disabled="isLastPage" class="pagination-btn">
              <i class="fas fa-chevron-right"></i>
            </button>
          </div>
        </div>
      ...
      <div v-if="activeTab === 'kanji'" class="kanji-section">
        ...
        <div v-else>
          <!-- Search Results Info -->
          <div v-if="searchQuery && searchQuery.trim()" class="search-results-info">
            <p>Tìm thấy <strong>{{ filteredKanjiTotal }}</strong> hán tự cho từ khóa "<strong>{{ searchQuery
            }}</strong>"</p>
            <p v-if="getSearchTotalPages() > 1" class="pagination-info">
              Hiển thị {{ searchCurrentPage * searchPageSize + 1 }} - {{ Math.min((searchCurrentPage + 1) *
                searchPageSize, filteredKanjiTotal) }} trong tổng số {{ filteredKanjiTotal }} kết quả
            </p>
          </div>

          <div class="items-grid">
            <div v-for="item in filteredKanji" :key="item.id" class="item-card kanji-card">
              <div class="card-header">
              ...
              <div class="card-content">
                <div class="kanji-grid">
                  <div><strong>Phát âm:</strong> {{ item.vietnamesePronunciation }}</div>
                  <div><strong>Onyomi:</strong> {{ item.onYomi }}</div>
                  <div><strong>Kunyomi:</strong> {{ item.kunYomi }}</div>
                </div>
                <div>
                  <div :class="['level-badge', item.jlptLevel.toLowerCase()]" style="margin-right: 4px;">{{
                    item.jlptLevel }}</div>
          ...
          <!-- Regular Pagination -->
          <div v-if="totalPages > 1 && (!searchQuery || !searchQuery.trim())" class="pagination">
            <button @click="loadKanji(currentPage - 1)" :disabled="currentPage === 0" class="pagination-btn">
              <i class="fas fa-chevron-left"></i>
            </button>
            ...
            <button @click="loadKanji(currentPage + 1)" :disabled="isLastPage" class="pagination-btn">
              <i class="fas fa-chevron-right"></i>
            </button>
          </div>
        </div>
      ...
      <div v-if="activeTab === 'sentences'" class="sentences-section">
        ...
        <div v-else>
          <!-- Search Results Info -->
          <div v-if="searchQuery && searchQuery.trim()" class="search-results-info">
            <p>Tìm thấy <strong>{{ filteredSentencesTotal }}</strong> mẫu câu cho từ khóa "<strong>{{ searchQuery
            }}</strong>"</p>
            <p v-if="getSearchTotalPages() > 1" class="pagination-info">
              Hiển thị {{ searchCurrentPage * searchPageSize + 1 }} - {{ Math.min((searchCurrentPage + 1) *
                searchPageSize, filteredSentencesTotal) }} trong tổng số {{ filteredSentencesTotal }} kết quả
            </p>
          </div>

          <div class="items-grid">
            <div v-for="item in filteredSentences" :key="item.id" class="item-card sentence-card">
              <div class="card-header">
                <div class="sentence-info">
                  <span>{{ item.grammarLink.label }} </span>
          ...
          <!-- Regular Pagination -->
          <div v-if="totalPages > 1 && (!searchQuery || !searchQuery.trim())" class="pagination">
            <button @click="loadSentences(currentPage - 1)" :disabled="currentPage === 0" class="pagination-btn">
              <i class="fas fa-chevron-left"></i>
            </button>
            ...
            <button @click="loadSentences(currentPage + 1)" :disabled="isLastPage" class="pagination-btn">
              <i class="fas fa-chevron-right"></i>
            </button>
          </div>
        </div>
    ...
    <div v-if="showModal" class="modal-overlay" @mousedown.self="closeModal">
      <div class="modal-content" @click.stop>
        ...
        <div class="modal-body">
          <!-- Vocabulary Form -->
          <div v-if="activeTab === 'vocabulary'">
            <Form ref="myForm" :validation-schema="vocabularySchema" class="form-group">
              <div class="form-row">
                <div class="form-field">
                  <label for="word">Từ vựng *</label>
                  <Field id="word" name="word" v-model="editingItem.word" type="text" placeholder="Nhập từ vựng"
                    required />
                  <ErrorMessage name="word" class="error-message" />
                </div>
                <div class="form-field">
                  <label for="reading">Cách đọc âm hán</label>
                  <input id="reading" v-model="editingItem.reading" type="text" placeholder="Nhập cách đọc" />
                </div>
              </div>

              <div class="form-row">
                <div class="form-field">
                  <label for="kanjiId">Liên kết Kanji (Vui lòng chọn đúng thứ tự)</label>
                  <Multiselect v-model="editingItem.kanjiLinks" mode="tags" placeholder="Gõ để tìm kiếm Kanji..."
                    open-direction="bottom" :object="true" :close-on-select="false" :min-chars="1"
                    :resolve-on-load="true" :delay="300" :searchable="true" :options="loadAvailableKanjis">
                  </Multiselect>
                </div>
              </div>
              ...
              <div class="form-row">
                <div class="form-field">
                  ...
                  <Field as="textarea" name="meaning" id="meaning" v-model="editingItem.meaning"
                    placeholder="Nhập nghĩa của từ" rows="3" required></Field>
                  <ErrorMessage name="meaning" class="error-message" />
                </div>
              </div>
          ...
          <div v-else-if="activeTab === 'kanji'">
            <Form ref="myForm" :validation-schema="kanjiSchema" class="form-group">
              <div class="form-row">
                <div class="form-field">
                  <label for="character">Ký tự Kanji *</label>
                  <Field id="character" name="character" v-model="editingItem.character" type="text"
                    placeholder="Nhập ký tự kanji" required />
                  <ErrorMessage name="character" class="error-message" />
                </div>
                <div class="form-field">
                  <label for="strokes">Số nét *</label>
                  <Field id="strokes" name="strokes" v-model="editingItem.strokes" type="number"
                    placeholder="Nhập số nét" min="1" />
                  <ErrorMessage name="strokes" class="error-message" />
                </div>
              </div>

              <div class="form-row">
                <div class="form-field">
                  <label for="onYomi">Onyomi (Âm On)</label>
                  <input id="onYomi" v-model="editingItem.onYomi" type="text" placeholder="Ví dụ: ジン, ニン" />
                </div>
                <div class="form-field">
                  <label for="kunYomi">Kunyomi (Âm Kun)</label>
                  <input id="kunYomi" v-model="editingItem.kunYomi" type="text" placeholder="Ví dụ: ひと" />
                </div>
              </div>
              ...
              <div class="form-row">
                <div class="form-field">
                  <label for="meaning">Nghĩa *</label>
                  <Field name="meaning" id="meaning" v-model="editingItem.meaning" placeholder="Nhập nghĩa của kanji"
                    rows="3" required></Field>
                  <ErrorMessage name="meaning" class="error-message" />
                </div>
              </div>
              ...
              <div class="form-row">
                <div class="form-field">
                  ...
                  <Field id="vietnamesePronunciation" name="vietnamesePronunciation"
                    v-model="editingItem.vietnamesePronunciation" type="text" placeholder="Nhập phát âm tiếng Việt" />
                </div>
              </div>

              <div class="form-row">
                <div class="form-field">
                  <label for="jlptLevel">Trình độ JLPT *</label>
                  <Field id="jlptLevel" name="jlptLevel" as="select" v-model="editingItem.jlptLevel">
                  ...
                  <ErrorMessage name="jlptLevel" class="error-message" />
                </div>
              </div>
            </Form>
          ...
          <div v-else-if="activeTab === 'grammar'">
            <Form ref="myForm" :validation-schema="grammarSchema" class="form-group">
              <div class="form-row">
                <div class="form-field">
                  <label for="structure">Cấu trúc ngữ pháp *</label>
                  <Field id="structure" name="structure" v-model="editingItem.structure" type="text"
                    placeholder="Nhập cấu trúc ngữ pháp (VD: ~ています)" />
                  <ErrorMessage name="structure" class="error-message" />
                </div>
              </div>
              ...
              <div class="form-row">
                <div class="form-field">
                  ...
                  <Field as="select" id="grammarType" name="grammarType" v-model="editingItem.grammarType">
                    <option value="" disabled>-- Chọn loại ngữ pháp --</option>
                  ...
                  <ErrorMessage name="grammarType" class="error-message" />
                </div>
              ...
              <div class="form-row">
                <div class="form-field">
                  ...
                  <Field as="textarea" id="explanation" name="explanation" v-model="editingItem.explanation"
                    placeholder="Nhập giải thích ngữ pháp" rows="3" required></Field>
              ...
              <div class="form-row">
                <div class="form-field">
                  ...
                  <Field as="textarea" id="example" name="example" v-model="editingItem.example"
                    placeholder="Nhập ví dụ sử dụng" rows="2"></Field>
                  <ErrorMessage name="example" class="error-message" />
                </div>
              </div>
            </Form>
          </div>
          ...
          <!-- Sentences Form -->
          <div v-else-if="activeTab === 'sentences'">
            <Form ref="myForm" :validation-schema="sampleSentenceSchema" class="form-group">
              <div class="form-row">
                <div class="form-field">
                  <label for="japanese">Câu tiếng Nhật *</label>
                  <Field id="japanese" name="japanese" v-model="editingItem.japanese" placeholder="Nhập câu tiếng Nhật"
                    rows="3"></Field>
                  <ErrorMessage name="japanese" class="error-message" />
                </div>
              </div>
              ...
              <div class="form-row">
                <div class="form-field">
                  ...
                  <Field id="vietnamese" name="vietnamese" v-model="editingItem.vietnamese"
                    placeholder="Nhập nghĩa tiếng Việt" rows="3"></Field>
                  <ErrorMessage name="vietnamese" class="error-message" />
                </div>
              </div>

              <div class="form-row">
                <div class="form-field">
                  <label for="grammarId">Liên kết ngữ pháp (tùy chọn)</label>
                  <Multiselect v-model="editingItem.grammarLink" mode="single" placeholder="Gõ để tìm kiếm ngữ pháp..."
                    open-direction="top" :close-on-select="true" :min-chars="1" :object="true" :resolve-on-load="true"
                    :delay="300" :searchable="true" :options="loadAvailableGrammars">
                  </Multiselect>
                </div>
              </div>
            </Form>
        ...
        <div class="modal-footer">
          ...
          <button @click="handleSubmit" class="btn-save">
    ...
    <ThePopup v-if="showDeleteConfirm" title="Xác nhận xóa" :message="deleteConfirmMessage" confirm-text="Xóa"
...
// schema validate when create/edit data
const vocabularySchema = toTypedSchema(
  z.object({
...
const kanjiSchema = toTypedSchema(
  z.object({
    character: z.string().min(1, 'Ký tự Kanji là bắt buộc'),
    meaning: z.string().min(1, 'Nghĩa là bắt buộc'),
    strokes: z.coerce.number().min(1, 'Số nét phải lớn hơn hoặc bằng 1'),
    jlptLevel: z.string().min(1, 'Vui lòng chọn cấp độ JLPT hợp lệ (N5-N1)')
  })
);

const sampleSentenceSchema = toTypedSchema(
  z.object({
    japanese: z.string().min(1, 'Câu tiếng Nhật là bắt buộc'),
    vietnamese: z.string().min(1, 'Nghĩa tiếng Việt là bắt buộc'),
  })
);

const grammarSchema = toTypedSchema(
  z.object({
    structure: z.string().min(1, 'Cấu trúc ngữ pháp là bắt buộc'),
    explanation: z.string().min(1, 'Giải thích là bắt buộc'),
    example: z.string().min(1, 'Ví dụ là bắt buộc'),
    grammarType: z.string().min(1, 'Loại ngữ pháp là bắt buộc'),
    jlptLevel: z.string().min(1, 'Vui lòng chọn cấp độ JLPT hợp lệ (N5-N1)')
  })
);
...
const loadSentences = async (page = 0) => {
  try {
    ...
    sentencesData.value = response.content.map(item => ({
      ...
      grammarLink: item.grammarLink || null,
      createdAt: new Date().toLocaleDateString('vi-VN')
    }))
...
const filteredGrammar = computed(() => {
  if (searchQuery.value && searchQuery.value.trim()) {
    ...
    const filtered = allGrammarData.value.filter(item => {
      ...
      const jlptLevelMatch = item.jlptLevel && item.jlptLevel.toLowerCase().includes(query)

      return structureMatch || explanationMatch || exampleMatch || pronunciationMatch || typeMatch || jlptLevelMatch
    })
...
const filteredSentences = computed(() => {
  if (searchQuery.value && searchQuery.value.trim()) {
    ...
    const filtered = allSentencesData.value.filter(item => {
      ...
      const grammarLinkMatch = item.grammarLink && item.grammarLink.label.toLowerCase().includes(query)

      return japaneseMatch || vietnameseMatch || grammarLinkMatch
    })
...
const getSearchPlaceholder = () => {
  switch (activeTab.value) {
    ...
    case 'sentences': return 'Search by câu tiếng Nhật, Việt or cấu trúc ngữ pháp...'
...
const getWordTypeLabel = (type) => {
  ...
  // Trả về tên tiếng Việt, nếu không tìm thấy thì trả về nguyên gốc tiếng Anh
  return types[type] || type
}
...
const loadAvailableKanjis = async (query) => {
  try {

    const response = await kanjiApi.getAll({ page: 0, size: 10, search: query || '' })
    return response.kanjis.map(k => ({
      value: k.kanjiId,
      label: `${k.characterName} - ${k.meaning}`
    }))
  } catch (err) {
...
const loadAvailableGrammars = async (query) => {
  try {
    const response = await grammarApi.getAll({ page: 0, size: 10, search: query || '' })
    return response.grammars.map(g => ({
      value: g.grammarId,
      label: `${g.structure} - ${g.explanation}`
    }))
  } catch (err) {
    console.error('Error loading available grammars:', err)
    return []
  }
}
...
const openCreateModal = async () => {
  ...
  } else if (activeTab.value === 'vocabulary') {
...
const editItem = async (item) => {
  ...
  // Load available grammars if editing sentences
...
const confirmDelete = async () => {
  try {
    ...
    if (result === 1 || result) {
      ...
      error.value = ''
      toast.success(`${getModalTitle().charAt(0).toUpperCase() + getModalTitle().slice(1)} đã được xóa thành công`);
    }
  } catch (err) {
  ...
  try {
    ...
    if (activeTab.value === 'kanji') {
      ...
      const vocabularyData = {
        ...
        kanjiLinks: item.kanjiLinks || null,
    ...
    toast.success(`${getModalTitle().charAt(0).toUpperCase() + getModalTitle().slice(1)} đã được lưu thành công`);
    closeModal()
  } catch (err) {
